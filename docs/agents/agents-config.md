...
内置 `ChatbotAgent` 是唯一执行后端。不同配置可分别用于诊疗和质控，配置之间不能互相委派。
...
编辑页只提交修改过的配置字段。修改可见的资源选择时，后端保留当前管理员不可访问的既有引用，并拒绝新增无权访问的引用；保留的引用维持原相对顺序，新选择追加到末尾。界面显示不可访问的选择数量。选择“清空全部”会显式移除全部已选引用，包括不可访问项。通过 API 更新 `config_json.context` 时，省略字段保留原值，显式 `null` 或空列表切换该字段的资源策略，非空列表修改可见部分并保留不可见的既有选择。
...
它不保存待办、文件、产物。运行态来自 LangGraph state 的 `agent_state`；能力声明只表达后端固定支持哪些 UI 入口。
...
- [单 Agent 范围](./subagents-management.md)
