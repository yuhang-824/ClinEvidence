...
本功能保证证据保留、显式审核和版本隔离，不承诺任意 PDF 无需人工就能准确理解。审核记录是人工判断，不是机器准确率证明；解析器证据充分的页后来改为由系统机器核验（见[异常驱动审核](./2026-09-20-exception-driven-structure-review.md)），机器结论与人工签核分开记录，本记录要求的人工判断对未机器核验的页仍然适用。
