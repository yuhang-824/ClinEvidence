...
ClinEvidence 基于 Yuxi 构建循证医助，采用单 Agent、文档检索与工具调用。知识库以本地 PDF 导入为主，目标是支持带证据的诊疗辅助与病历质控。
...
- 让智能体调用知识库、MCP、Skills 和沙盒工具，完成多步骤任务。
...
- **智能体（Agent）**：由模型、提示词、工具、知识库、Skills 和 MCP 共同组成的运行单元。
- **知识库**：保存文档并提供解析、分块、向量检索和来源信息。当前使用 Milvus 本地文档知识库。
...
Yuxi 使用 LangGraph 组织智能体运行。管理员可以配置模型、系统提示词、工具、MCP、Skills 和上下文压缩策略。运行时间较长的任务由独立 worker 执行，事件通过 SSE 发送到前端，结果和运行状态保存到 PostgreSQL。

工具可以读取和修改 Workdir 中的文件、执行沙盒命令或请求用户审批。完成的文件可以在界面中预览和下载。
